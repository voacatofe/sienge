...
async function testSyncWithErrors() {
  ...
  console.log('Este teste irá sincronizar as entidades problemáticas identificadas nos logs:\n');
  ...
  try {
    ...
    if (data.errorSummary) {
      ...
      // Erros por entidade
      if (summary.errorsByEntity && Object.keys(summary.errorsByEntity).length > 0) {
        console.log('📋 DISTRIBUIÇÃO DE ERROS POR ENTIDADE:');
        const entities = Object.entries(summary.errorsByEntity)
          .sort(([, a], [, b]) => (b as number) - (a as number));

        for (const [entity, count] of entities) {
          const percentage = ((count as number) / summary.totalErrors * 100).toFixed(1);
          const bar = '█'.repeat(Math.floor((count as number) / summary.totalErrors * 30));
          console.log(`  ${entity.padEnd(20)} ${String(count).padStart(5)} erros (${percentage}%) ${bar}`);
        }
      }
      ...
      if (summary.errorsByType && Object.keys(summary.errorsByType).length > 0) {
        ...
        const types = Object.entries(summary.errorsByType)
          .sort(([, a], [, b]) => (b as number) - (a as number));

        for (const [type, count] of types) {
          const percentage = ((count as number) / summary.totalErrors * 100).toFixed(1);
          console.log(`  ${type.padEnd(25)} ${String(count).padStart(5)} (${percentage}%)`);
        }
      }
    ...
    for (const result of data.results) {
      if (previousErrors[result.entity]) {
        ...
        console.log(`  ${result.entity.padEnd(20)} ${String(prev).padStart(4)} → ${String(curr).padStart(4)} (${symbol} ${Math.abs(diff)}) ${color}`);
      }
    }

    console.log('\n' + '='.repeat(80));
    console.log('Teste concluído! Verifique o diretório logs/sync-errors/ para o log completo.');
    console.log('='.repeat(80));
