...
export function ConfigurationSection({
  ...
  const handleCancelEdit = () => {
    ...
    if (existingCredentials) {
      reset({
        subdomain: existingCredentials.subdomain,
  ...
  const syncAllEndpoints = async () => {
    // Lista de endpoints com permissão de acesso
    // Apenas endpoints que não retornam 403 (Permission denied)
    const endpoints = [
    ...
    const endpointsByPhase = validEndpoints.reduce(
      (acc, endpoint) => {
    ...
    const phases = Object.keys(endpointsByPhase)
      .map(Number)
      .sort((a, b) => a - b);
    ...
    if (errorCount === 0) {
      ...
      setIsEditMode(false); // Sair do modo de edição após sucesso
    } else if (successCount > 0) {
      setSubmitStatus('success');
      setIsEditMode(false); // Sair do modo de edição após sucesso parcial
      setErrorMessage(
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-8">
      ...
      {/* Seção de visualização quando configurado */}
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-green-700 text-sm">
                  Sua API Sienge está configurada e pronta para sincronização
                </p>
              </div>
            </div>
            <button
              ...
              className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-md transition-colors focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          {existingCredentials && (
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="text-green-900 bg-white px-3 py-2 rounded border border-green-200">
                  {existingCredentials.subdomain}.sienge.com.br
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-green-800 mb-1">
      ...
      {/* Formulário só aparece se não configurado OU em modo de edição */}
      {(!isConfigured || isEditMode) && (
          ...
          <div className="flex gap-3 pt-2">
            ...
            {isEditMode && (
              <button
                ...
                disabled={isSubmitting}
                className="flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Cancelar
              </button>
            )}
